Feature: Pre-auth request validation
  As a Merchant,
  I want the Proxy API Gate to validate my Pre-auth authorizing trx requests
  ...
  @DISE-346 @DISE-538
  Scenario Outline: Pre-auth transaction - validate error code <fieldErrorCode> and associated error message
...
#      | payment.cardHolder       | {EMPTY}                          | 16010          | Defect DISE-695
  ...
  @DISE-346 @DISE-538
  Scenario Outline: Pre-auth transaction - paymentInstrumentId and cardholder data invalid combinations
    Given I send a 'preauth' transaction request with specific fields:
      | currency                    | USD                   |
  ...
  @DISE-346 @DISE-538
  Scenario: Pre-auth transaction - paymentInstrumentId and no cardholder data
    Given I send a 'preauth' transaction request with specific fields:
      | currency                    | USD         |
  ...
  @DISE-346 @DISE-538
  Scenario Outline: Pre-auth transaction - validate basket field with error code <fieldErrorCode> and associated error message
    Given I send a 'preauth' transaction request with specific fields:
      | currency        | USD          |
      | <fieldPath>     | <fieldValue> |
  ...
  @DISE-346 @DISE-538
  Scenario: Pre-auth transaction - validate error code 16008 and associated error message
    Given I send a 'preauth' transaction request with specific fields:
      | currency            | USD                    |
  ...
  @DISE-346 @DISE-538
  Scenario: Pre-auth transaction - validate multiple error codes and associated error messages
    Given I send a 'preauth' transaction request with specific fields:
      | currency                 | USD                      |
      | initialAmount            | {LONG:19}                |
      | product                  | {STRING:5}               |
      ...
      | basket.name              | {STRING:130}             |
      | basket.articleNumber     | {STRING:260}             |
      | basket.totalPrice        | {-LONG:19}               |
      | basket.totalPriceWithTax | {LONG:19}                |
      | basket.unitPrice         | {LONG:19}                |
      | basket.unitPriceWithTax  | {-LONG:19}               |
      | basket.tax               | {INT:4}                  |
      | basket.quantity          | {-INT:1}                 |
      | customer                 | {STRING:260}             |
      | persona                  | {STRING:260}             |
    ...
    Then the status code should be '400'
    And the 'preauth' response contains error codes for all invalid fields

  @DISE-346 @DISE-538
  Scenario Outline: Pre-auth transaction - validate values for fields with sets of accepted inputs
    Given I send a 'preauth' transaction request with specific fields:
      | currency    | USD          |
      | <fieldPath> | <fieldValue> |
    Then the status code should be '200'
  ...
  @DISE-346 @DISE-538
  Scenario: Pre-auth transaction - validate accepted values for optional fields
    Given I send a 'preauth' transaction request with specific fields:
      | currency                              | USD                          |
      | async.notifications.notificationState | {ARRAY:CREATED}              |

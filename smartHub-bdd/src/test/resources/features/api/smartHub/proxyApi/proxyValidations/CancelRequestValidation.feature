Feature: Cancel request validation
  As a Merchant,
  I want the Proxy API Gate to validate my Cancel referencing trx requests
  ...
  @DISE-933 @DISE-1253
  Scenario Outline: Cancel transaction - validate error code <fieldErrorCode> and associated error message
  ...
  @DISE-933 @DISE-1253
  Scenario Outline: Cancel transaction - validate basket field with error code <fieldErrorCode> and associated error message
    Given I send a 'preauth' transaction request with specific fields:
      | initialAmount | {LONG:3..999} |
      | currency      | USD           |
    When I 'cancel' a transaction with specific fields:
  ...
  @DISE-933 @DISE-1253
  Scenario: Cancel transaction - validate multiple error codes and associated error messages
    Given I send a 'preauth' transaction request with specific fields:
      | initialAmount | {LONG:3..999} |
      | currency      | USD           |
    When I 'cancel' a transaction with specific fields:
...
#      | basket[0].unitPriceWithTax  | {-INT:2}     |
    ...
    Then the status code should be '400'
    And the 'cancel' response contains error codes for all invalid fields

  @DISE-933 @DISE-1253
  Scenario Outline: Cancel transaction - validate values for fields with sets of accepted inputs
    Given I send a 'preauth' transaction request with specific fields:
      | initialAmount | {LONG:3..999} |
      | currency      | USD           |
    When I 'cancel' a transaction with specific fields:
  ...
  @DISE-933 @DISE-1253
  Scenario: Cancel transaction - validate accepted values for optional fields
    Given I send a 'preauth' transaction request with specific fields:
      | initialAmount | {LONG:3..999} |
      | currency      | USD           |
    When I 'cancel' a transaction with specific fields:

Feature: Capture request validation
  As a Merchant,
  I want the Proxy API Gate to validate my Capture referencing trx requests
...
#      | basket[0].unitPriceWithTax  | {-INT:2}     |
    ...
    Then the status code should be '400'
    And the 'capture' response contains error codes for all invalid fields
